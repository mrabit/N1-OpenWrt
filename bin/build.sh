#!/usr/bin/env bash
# One-shot native build for the N1 image (no Docker).
#
# Clones ImmortalWrt, runs diy.sh, applies the N1 overlay/config and compiles,
# publishing the firmware to $ARTIFACT_DIR/bin/targets (default BUILD_ROOT/output;
# set ARTIFACT_DIR=./output to pull it into the repo). The openwrt tree and all
# caches live under BUILD_ROOT, which MUST be on a native ext4/xfs/btrfs volume:
# OpenWrt's tree has huge numbers of small parallel writes and hardlinks that
# corrupt on a virtiofs/macOS bind mount (e.g. perl/ncurses host builds failing
# randomly). Keep the repo where it is; only BUILD_ROOT must be native.
#
# Usage:
#   bin/build.sh                              # foreground, logs to $BUILD_ROOT/build.log
#   BUILD_ROOT=/mnt/ssd/owrt bin/build.sh     # different native scratch volume
#   ALL_PROXY=socks5h://host:port bin/build.sh
#   JOBS=4 bin/build.sh                       # cap compile parallelism
set -euo pipefail
...
# Re-stamp the N1 overlay + .config onto the tree and regenerate defconfig. Cheap
# and idempotent, so both full and incremental paths run it — an incremental
# build thus picks up package/overlay edits without touching feeds or diy.sh.
apply_overlay() {
    rm -rf files .config
    cp -r "$REPO_ROOT/armsr/armv8/N1/files" ./files
    cp "$REPO_ROOT/armsr/armv8/N1/.config" ./.config
...
if [ "$INCREMENTAL" = "1" ] && [ -f "$OUTDIR/openwrt/.config" ]; then
    ...
    update_feeds
    bash "$REPO_ROOT/armsr/armv8/diy/diy.sh"
    update_feeds
