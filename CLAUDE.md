...
Both profiles build from one unified workflow: `.github/workflows/build.yml`. A matrix `build` job compiles N1 and x86 in parallel (they share ~95% of the steps); a single `release` job then downloads every profile's image artifact and publishes **one combined dated release** (`firmware_<date>`, all images as assets in one place; no rolling `latest` tag). N1 images are **additionally** pushed to the fixed `OpenWrt` tag that `luci-app-amlogic` OTA reads — x86 images never land there, so 晶晨宝盒 can't offer the wrong image. That `OpenWrt` release is fully reset each run (`gh release delete --cleanup-tag` drops the release **and** its git tag), then recreated at the current commit, so the OTA tag always points at the freshest N1 image. `workflow_dispatch` takes a `profile` input (`all`/`N1`/`x86`) to build a single profile on demand.
